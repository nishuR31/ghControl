...
export type GithubOpJob =
  ...
  | { type: 'create_issue';  token: string; owner: string; repo: string; title: string; body?: string; labels?: string[] }
  | { type: 'close_issue';   token: string; owner: string; repo: string; number: number }
  | { type: 'merge_pr';      token: string; owner: string; repo: string; number: number; method?: string }
  | { type: 'create_branch'; token: string; owner: string; repo: string; branch: string; from: string }
  | { type: 'delete_branch'; token: string; owner: string; repo: string; branch: string }
  | { type: 'create_release';token: string; owner: string; repo: string; tag: string; name: string; body?: string; draft?: boolean; prerelease?: boolean }
  | { type: 'create_repo';   token: string; name: string; description?: string; private?: boolean; autoInit?: boolean }
  | { type: 'delete_repo';   token: string; owner: string; repo: string }
  | { type: 'fork_repo';     token: string; owner: string; repo: string; org?: string }
...
export async function enqueueRepoSync(token: string, userId: string) {
  return repoSyncQueue.add('sync', { token, userId }, {
